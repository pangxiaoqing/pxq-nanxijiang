<template>
        <div class="pickOrder">
            <pull-to :bottom-load-method="bottom_load" :top-load-method="refresh" class="jiazai">
                <div class="singleOrder"  v-for="item in data_list" :key="item.id"   @click="clickSingle(item.STATUS,item.id)">
                  <div class="orderTop">
...
export default {
  ...
  mounted() {
    ...
    if (this.type == 1) {
      ...
      child[1].style.color = "#40B1FF";
      child[1].style.fontWeight = "600";
    }
  },
  components: {
    PullTo
